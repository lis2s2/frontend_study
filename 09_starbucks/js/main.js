...
// const inputEl = document.querySelector('.search input'); // 문서 전체에서 찾지 말고 아래와 같이 최적화
...
//   gsap.to(window, 0.6, {
//     scrollTo: 0 // 페이지의 0px지점(최상단)으로 이동, ScrollToPlugin을 연결하여 사용 가능한 옵션
//   });
// })
...
window.addEventListener('scroll', function () {
  ...
  // 페이지 스크롤 위치가 500px 넘지 않으면 배지 요소를 보이기
...
fadeEls.forEach(function (fadeEl, index) {
  ...
  gsap.to(fadeEl, 1, {
  opacity: 1,
  delay: 0.7*(index + 1) // 0.7s, 1.4s, 2.8s
  });
});
...
const thisYear = document.querySelector('.this');
thisYear.textContent = new Date ().getFullYear(); // 현재 연도 정보가 숫자 데이터로 변환됨
