...
function App() {
  ...
  const [posts, setPosts] = useState([
    '리액트 잘 쓰려면?', '자바스크립트 핵심 문법', 'CSS 스타일링 가이드'
  ]);
  ...
  return (
    <>
      ...
      <div className='inner'>
            ...
            <div key={index} className='list'
              ...
              <div className='toolbar'>
                <span onClick={(e) => {
                  ...
                  const likeCounts = [...likeCount];
                  likeCounts[index]++;
                  setLikeCount(likeCounts);
                }}>💕 {likeCount[index]}</span>

                {/* 포스트 삭제하기 */}
                {/* 퀴즈: 포스트마다 삭제 버튼 & 기능 만들기 */}
                <span onClick={(e) => {
                  e.stopPropagation();
        ...
        {/* 포스트 등록하기 */}
        {/* 퀴즈: input에 제목 입력 후 등록 버튼 누르면 맨 앞에 새로운 포스트 추가
        ...
        */}
        ...
        <button type="button" onClick={() => {
          ...
          const copy = [newPost, ...posts];
          setPosts(copy);

          setNewPost(''); // 새롭게 포스팅 하고서 인풋란 비우기 위해
