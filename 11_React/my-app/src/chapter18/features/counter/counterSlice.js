...
// state 초기값으로 원시값, 배열, 객체 등 모든 데이터 타입이 사용 가능
// 주로 객체 형태로 사용함
...
const counterSlice = createSlice({
  ...
  reducers: { // reducer들을 정의
    increment: (state) => { // 첫번재 파라미터: 현재 state를 받아옴(실제로는 state의 복사본)
    ...
    incrementByAmount: (state, action) => { // 첫번째 파라미터: 전역 state 두번째 파라미터: action 객체 = { type: 발생한 액션의 타입, palyload: 화물, 전달 데이터 }
...
// 3) 수정을 원할 때 그 리듀서 함수를 실행해달라고 store에 요청을 해야됨
